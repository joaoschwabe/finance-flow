  ...
  (
    {
      ...
      children,
      searchable = true,
      searchPlaceholder = 'Busque por um ícone...',
    ...
    const renderVirtualContent = useCallback(() => {
      if (filteredIcons.length === 0) {
        return <div className="text-center text-gray-500">Nenhum ícone encontrado.</div>;
      }
      ...
      return (
        <div
          ...
          {virtualizer.getVirtualItems().map((virtualItem: VirtualItem) => {
            const itens = virtualItems[virtualItem.index];
            ...
            return (
              <div key={virtualItem.key} data-index={virtualItem.index} style={itemStyle}>
                <div className="grid w-full grid-cols-5 gap-2">{itens.map(renderIcon)}</div>
              </div>
            );
          })}
        </div>
